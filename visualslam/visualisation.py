...
class Scene3D:
    ...
    def _update_current_camera_visualisation(self, frame: Frame):
        ...
        if frame.pose_w_c is not None:
            self._current_camera_actors = add_frustum(self._plotter, frame.pose_w_c, self._camera_model, frame.colour_image) + add_axis(self._plotter, frame.pose_w_c)

    def add_keyframe(self, frame: Frame):
        self._keyframe_actors += add_frustum(self._plotter, frame.pose_w_c, frame.camera_model, frame.colour_image) + add_axis(self._plotter, frame.pose_w_c)
...
class ArRenderer:
    ...
    def update(self, frame: Frame):
        ...
        _, ar_rendering = self._plotter.show(return_cpos=True, return_img=True, screenshot=True, interactive_update=True)
...
def draw_detected_keypoints(vis_img, frame: Frame):
    for kp in frame.keypoints:
        cv2.drawMarker(vis_img, tuple(map(round, kp.pt)), (0, 255, 0), cv2.MARKER_CROSS, 5)
